...
pub fn run_validation_pipeline(
    ...
    if let Ok(repo_id) =
        conn.query_row::<String, _, _>(repo_id_query, [execution_id], |row| row.get(0))
    {
    ...
    if let Ok(head_out) = git_head {
        if head_out.status.success() {
            ...
            pass_step!(
                "Gate A: Snapshot",
                start_a.elapsed().as_millis() as u64,
                format!("HEAD captured: {}", commit)
            );
        } else {
            fail_pipeline!(
                "Gate A: Snapshot",
    ...
    let ownership_hash_path = Path::new(worktree_path)
        .join(".nexora")
        .join("ownership.hash");
    if let Ok(disk_hash) = fs::read_to_string(&ownership_hash_path) {
        ...
        if disk_hash.trim() != expected_hash {
            fail_pipeline!(
                "Gate B: Ownership Hash Drift",
                Some(1),
                start_b.elapsed().as_millis() as u64,
                format!(
                    "Contract drift detected! Expected {}, got {}",
        ...
        fail_pipeline!(
            ...
            None::<i32>,
            start_b.elapsed().as_millis() as u64,
            "ownership.hash missing".to_string()
        );
    }

    let ownership_result =
        crate::swarm_ownership::validate_ownership(worktree_path.to_string(), allowed_patterns);
    match ownership_result {
        Ok(res) if res.is_valid => {
            pass_step!(
                "Gate B: Ownership Validation",
                start_b.elapsed().as_millis() as u64,
                format!("{} files modified safely", res.modified_files.len())
            );
        }
        Ok(res) => {
            fail_pipeline!(
                ...
                Some(1),
                start_b.elapsed().as_millis() as u64,
                format!("Violated files: {:?}", res.violated_files)
            );
        }
        Err(e) => {
            fail_pipeline!(
    ...
    if status_out.is_err() || !status_out.unwrap().status.success() {
        fail_pipeline!(
            "Gate C: Git Integrity (Fast)",
            Some(1),
            start_c.elapsed().as_millis() as u64,
    ...
    if profile.deep_git_integrity {
        ...
        if fsck_out.is_err() || !fsck_out.unwrap().status.success() {
            fail_pipeline!(
                "Gate C: Git Integrity (Deep)",
                Some(1),
                start_c.elapsed().as_millis() as u64,
                "git fsck failed. Repository corrupted.".to_string()
            );
        }
    }
    pass_step!(
        "Gate C: Git Integrity",
        start_c.elapsed().as_millis() as u64,
        "Git tree is clean and intact.".to_string()
    );
    ...
    match run_script(
        ...
        Ok((true, _, out)) => pass_step!(
            "Gate D: Type Check",
            start_d.elapsed().as_millis() as u64,
            out
        ),
        Ok((false, code, out)) => fail_pipeline!(
            "Gate D: Type Check",
            code,
            start_d.elapsed().as_millis() as u64,
            out
    ...
    match run_script(
        ...
        Ok((false, code, out)) => fail_pipeline!(
            "Gate E: Lint",
            code,
            start_e.elapsed().as_millis() as u64,
            out
        ),
        Err(e) => fail_pipeline!(
            "Gate E: Lint",
    ...
    match run_script(
        ...
        Ok((false, code, out)) => fail_pipeline!(
            "Gate F: Test",
            code,
            start_f.elapsed().as_millis() as u64,
            out
        ),
        Err(e) => fail_pipeline!(
            "Gate F: Test",
...
pub async fn run_validation_async(
    ...
    let _permit = VALIDATION_SEMAPHORE
        .acquire()
        .await
        .map_err(|e| e.to_string())?;
