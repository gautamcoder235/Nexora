...
pub fn get_all_agents(app_handle: AppHandle) -> Result<Vec<Agent>, String> {
    ...
    let mut stmt = conn.prepare("SELECT id, name, command, version, enabled FROM agents")
    ...
    for row_res in agent_rows {
        ...
        let mut cap_stmt = conn.prepare("SELECT capability FROM agent_capabilities WHERE agent_id = ?1")
            .map_err(|e| e.to_string())?;
        let caps = cap_stmt.query_map([&id], |row| row.get::<_, String>(0))
            .map_err(|e| e.to_string())?;
...
#[tauri::command]
pub fn update_agent_status(app_handle: AppHandle, id: String, enabled: bool) -> Result<bool, String> {
    let db_state: State<DbState> = app_handle.state();
    let conn_guard = db_state.0.lock().map_err(|_| "Failed to lock DB".to_string())?;
    let conn = conn_guard.as_ref().ok_or("Database not initialized")?;
