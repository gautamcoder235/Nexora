...
impl CliDriver for AgyDriver {
    fn metadata(&self) -> DriverMetadata {
        DriverMetadata {
            id: "agy".to_string(),
            display_name: "Antigravity CLI (agy)".to_string(),
            executable: "agy".to_string(),
            supported_versions: ">=0.1.0".to_string(),
            platform: std::env::consts::OS.to_string(),
            capabilities: self.capabilities(),
            session_strategy: "Native flag session resume".to_string(),
            discovery_strategy: "Logs regex parsing".to_string(),
            launch_strategy: "AGY command execution".to_string(),
        }
    }

    fn detect(&self) -> Result<bool, DriverError> {
        Ok(crate::drivers::sdk::is_executable_in_path("agy"))
    }
