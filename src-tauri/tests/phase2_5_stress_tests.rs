...
fn test_log_flood() {
    ...
    assert!(
        elapsed.as_secs() < 30,
        "Log Flood took too long (SQLite bottlenecked)"
    );
    println!(
        "Log Flood Passed: 200,000 logs across 20 threads in {:?}",
        elapsed
    );
}
...
fn test_ownership_scale() {
    ...
    for i in 0..10000 {
        std::fs::write(
            target_dir.join(format!("SettingsPanel{}.tsx", i)),
            "content",
        )
        .unwrap();
    }
    ...
    std::process::Command::new("git")
        .current_dir(&repo_dir)
        .args(["commit", "-m", "init", "--author=Test <test@example.com>"])
        .output()
        .unwrap();
    ...
    assert!(
        result.is_valid,
        "Ownership validation failed on 10,000 valid files"
    );
    assert_eq!(result.modified_files.len(), 10000);
    assert!(
        validate_elapsed.as_secs() < 5,
        "Ownership validation took too long"
    );
    ...
    assert!(
        !worst_case_result.is_valid,
        "Ownership validation failed to catch 1 invalid file"
    );
    assert_eq!(worst_case_result.violated_files.len(), 1);
    assert_eq!(worst_case_result.violated_files[0], "App.tsx");
    assert!(
        worst_case_elapsed.as_secs() < 5,
        "Worst-case ownership validation took too long"
    );
