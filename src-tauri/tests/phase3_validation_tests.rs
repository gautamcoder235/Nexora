...
fn test_ownership_hash_drift() {
    ...
    assert_ne!(
        disk_hash.trim(),
        expected_hash,
        "Drift detection failed, hashes match!"
    );
}
...
fn test_validation_pipeline_db_schema() {
    ...
    // Insert sample data
    conn.execute(
        "INSERT INTO repositories (id, path, name) VALUES ('repo1', '/path', 'name')",
        [],
    )
    .unwrap();
    ...
    let deep: bool = conn
        .query_row(
            "SELECT deep_git_integrity FROM validation_profiles WHERE repository_id = 'repo1'",
            [],
            |r| r.get(0),
        )
        .unwrap();
