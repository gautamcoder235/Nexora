...
fn test_repository_switching() {
    ...
    for i in 1..=3 {
        ...
        swarm_db::insert_repository_if_missing(&conn, &repo_id, &format!("Repo {}", i), repo_path)
            .unwrap();
        ...
        swarm_db::insert_execution(&conn, &exec_id, &task_id, "agent-1", &wt_id, "running")
            .unwrap();
        ...
        assert!(
            result.is_valid,
            "Repo {} should be valid, but got violated: {:?}",
            i, result.violated_files
        );
    }
    ...
    for exec in execs {
        let mut stmt = conn
            .prepare("SELECT repository_id FROM worktrees WHERE id = ?1")
            .unwrap();
        let repo_id: String = stmt
            .query_row([&exec.worktree_id], |row| row.get(0))
            .unwrap();
...
fn test_large_repository() {
    ...
    assert!(
        elapsed.as_secs() < 5,
        "validate_ownership took too long: {:?}",
        elapsed
    );
...
fn test_sqlite_corruption_recovery() {
    ...
    assert!(
        conn_result.is_ok(),
        "Opening the file handle shouldn't fail even if corrupted"
    );
    ...
    assert!(
        ...
        "run_migrations should fail gracefully, not panic"
    );
    println!(
        "Gracefully caught DB corruption error: {:?}",
        migrate_result.unwrap_err()
    );
