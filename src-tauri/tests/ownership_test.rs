...
fn test_validate_ownership() {
    ...
    let result =
        validate_ownership(temp_dir_str, allowed_patterns).expect("validate_ownership failed");
    ...
    assert!(
        result.violated_files.contains(&"src/main.rs".to_string()),
        "violated_files should contain src/main.rs"
    );

    // 8. Asserts that `modified_files` contains both files.
    assert!(
        result.modified_files.contains(&"src/main.rs".to_string()),
        "modified_files should contain src/main.rs"
    );
    assert!(
        result
            .modified_files
            .contains(&"src/components/settings/index.ts".to_string()),
        "modified_files should contain src/components/settings/index.ts"
    );
